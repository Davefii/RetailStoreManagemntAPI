...
app.UseHttpsRedirection();
//Rate Limiting Middleware to protect against brute-force attacks on authentication endpoints.
app.UseRateLimiter();
...
app.UseAuthorization();
// Custom middleware to log 403 Forbidden responses for security monitoring.
app.Use(async (context, next) =>
{
    await next();
